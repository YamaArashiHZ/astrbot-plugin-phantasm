...
_KEEP_HOST_CONFIG = (
    ...
    "LogConfig", "Mounts", "Privileged", "ReadonlyRootfs", "Tmpfs", "Ulimits",
)
...
class DockerControl:
    ...
    async def recreate_with_env(self, name: str, env_updates: dict[str, str],
        ...
        new_cfg: dict[str, Any] = {"Image": image_override or info.get("Image") or cfg.get("Image"),
                                   "Env": new_env}
        for key in ("Labels", "WorkingDir", "ExposedPorts", "Entrypoint", "Cmd", "User", "Hostname"):
            if cfg.get(key):
                new_cfg[key] = cfg[key]
        ...
        try:
            async with self._client() as c:
                ...
                r = await c.post("/containers/create", params={"name": name},
                                 json={"Image": new_cfg["Image"], "Env": new_env,
                                       "Labels": new_cfg.get("Labels") or {},
                                       "HostConfig": new_host,
                                       **{k: new_cfg[k] for k in
                                          ("WorkingDir", "ExposedPorts", "Entrypoint", "Cmd", "User", "Hostname")
                                          if k in new_cfg}})
                ...
                if st.status_code not in (204, 304):
                    return False, f"新容器启动失败（HTTP {st.status_code}）：{st.text[:300]}"
                # 成功：删掉备份容器
